...
class Game {
    ...
    update() {
        ...
        this.pieces.forEach((elm) => {
            ...
            if (this.checkKingMoves(this.isWhiteTurn).length > 0) {
                const kingPos = this.pieces.find(elm => elm && elm.isWhite === this.isWhiteTurn && elm.type == 0).pos
                document.querySelector(`.row[id="${kingPos[0] + 1}"] .cell[id="${kingPos[1] + 1}"]`).style = "background-color:red"
                this.check = true
                this.checkCheckMate()
            }
            if (this.checkMate) {
    ...
    returnAv(id, returnType, pieces = this.pieces, board = this.board) {
        ...
        switch (item.type) {
            case 0:
                ...
                if (castleRight && !this.check) {
                    returnedValue.push([item.pos[0] + 2, item.pos[1]])
                }
                if (castleLeft && !this.check) {
                    returnedValue.push([item.pos[0] - 2, item.pos[1]])
                }
                break;
            case 1:
    ...
    checkCheckMate() {
        if (this.check) {
            let avMoves = [];
            let coloredPieces = this.pieces.filter(el => el.isWhite == this.isWhiteTurn)
